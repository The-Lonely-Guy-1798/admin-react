...
export const createChapter = async (chapterData) => {
  try {
    const chapterToCreate = {
      ...
      authorId: 'admin', // Since only admin can create chapters
      wordCount: calculateWordCount(chapterData.content || '')
    };

    const docRef = await addDoc(collection(db, COLLECTION_NAME), chapterToCreate);
...
export const updateChapter = async (chapterId, updateData) => {
  try {
    ...
    Object.keys(chapterToUpdate).forEach(key => {
      if (chapterToUpdate[key] === undefined) {
        delete chapterToUpdate[key];
      }
    });

    const docRef = doc(db, COLLECTION_NAME, chapterId);
    await updateDoc(docRef, chapterToUpdate);
